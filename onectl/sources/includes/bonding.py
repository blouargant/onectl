...
class Interface(object):
	...
	def get_default_gateway_dev(self):
	...
	def get_network_addr(self, ifname, type):
	...
	def get_mac_addr_raw(self, ifname):
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		return fcntl.ioctl(
			s.fileno(),
			SIOCGIFHWADDR,
	...
	def is_iface_flags(self, ifname, type):
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	...
	def set_iface_flag(self, ifname, flag, flags = None):
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		if not flags:
			flags = 0
	...
	def get_iface_link_status(self, ifname):
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	...
	def get_slave_iface_list(self, ifname):
		try:
	...
	def is_iface_slave(self, ifname):
		return is_iface_flags(ifname, IFF_SLAVE)
	
	def is_iface_master(self, ifname):
		return is_iface_flags(ifname, IFF_MASTER)
	
	def is_iface_up(self, ifname):
		return is_iface_flags(ifname, IFF_UP)
	
	def is_iface_loopback(self, ifname):
